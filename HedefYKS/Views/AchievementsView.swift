...
struct AchievementsView: View {
    ...
    private var allBadges: [BadgeItem] {
        ...
        let completedTotal = dataManager.completedTopicsCount()
        let totalTotal = dataManager.totalTopicsCount()
        let overallPercent = dataManager.overallPercentage
        
        let completedTYT = dataManager.completedTopicsCount(for: "TYT")
        let totalTYT = dataManager.totalTopicsCount(for: "TYT")
        
        let aytSec = dataManager.currentField == .dil ? "YDT" : "AYT"
        let completedAYT = dataManager.completedTopicsCount(for: aytSec)
        let totalAYT = dataManager.totalTopicsCount(for: aytSec)
        
        // MARK: - 1. Genel Kilometre Taşları
        ...
        // MARK: - 2. Ders Bazlı Rozetler
        for course in dataManager.courses {
            let courseCompleted = course.topics.filter { dataManager.getState(for: $0.id) == .completed }.count
            ...
            list.append(BadgeItem(
                ...
                badgeColor: dataManager.themeColor
            ))
        }
